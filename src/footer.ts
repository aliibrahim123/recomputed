import { Component, registry, Signal, type $temp } from "@neocomp/full/comp-base";
...
export class Footer extends Component {
	constructor (rootHref?: string) {
		super();
		const { $temp } = this.createTop();
		$temp`<div id="footer" class="dark:bg-slate-900 bg-gray-200 py-1.5 px-3 flex flex-row gap-3">`;

		const selectedClasses = tw`
			bg-white ring inset-ring ring-gray-950/10 inset-ring-white/10 
			dark:bg-gray-700 dark:text-white dark:ring-transparent
		`;

		// theme selector
		let theme = this.signal(localStorage.theme || 'system');
		this.effect(() => {
		...
		">`;
		for (let style of ['system', 'light', 'dark'] as const) {
			$temp`<span
				class="rounded-full *:size-7" on:click=${() => theme.value = style}
				${el => this.effect(() => {
					if (theme.value === style) el.classList.add(...selectedClasses);
					else el.classList.remove(...selectedClasses)
				})}
			>`;
			this[style + 'Svg' as 'darkSvg']($temp);
			$temp`</span>`;
		}
		$temp`</span>`;
		
		// content width selector
		let contentWidth = this.signal(localStorage.getItem('contentWidth') || '100ch');
		this.contentWidth = contentWidth as any;
		this.effect(() => {
			localStorage.setItem('contentWidth', contentWidth.value);
			updateTheme();
		});
		
		$temp`<span class="
			inline-grid grid-cols-3 rounded-full gap-0.5
			bg-gray-950/8 items-center text-gray-950 dark:bg-white/10 dark:text-white"
		>`;
		for (let style of ['100ch', '120ch', '100%'] as const) $temp`<span
			class="rounded-full py-0.25 px-1 ml-1" on:click=${() => contentWidth.value = style}
			${el => this.effect(() => {
				if (contentWidth.value === style) el.classList.add(...selectedClasses);
				else el.classList.remove(...selectedClasses)
			})}
		>${style}</span>`;
		$temp`</span>`;
		...
		// root link
		$temp`<a .href=${rootHref} class="mr-3 text-xl my-auto font-mono ml-auto">ReComputed</a>`;
		
		$temp`</div>`;
		this.fireInit();
	}
	contentWidth: Signal<'100%' | '100ch' | '120ch'>;

	systemSvg ($temp: $temp) {
		$temp`<svg viewBox="0 0 28 28" fill="none">
